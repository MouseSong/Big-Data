 ...
 *  10,ACCOUNTING,NEW YORK
	...
	map-> “部门名称”，“员工姓名，入职日期”
	reduce -> 比较出最早的时间 -> “部门名称”，“员工姓名”
 * */
public class OldestEmpNameOfDept extends Configured implements Tool{
		...
		protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context)
			...
			if(deptInfo.containsKey(empInfo[7])){
				if(null != empInfo[5] && !"".equals(empInfo[5].toString())){
					context.write(new Text(deptInfo.get(empInfo[7])), new Text(empInfo[1] + "," + empInfo[4]));
				}
			}
		}
	}
	...
	private static class ReduceClass extends Reducer<Text, Text, Text, Text>{
		...
		protected void reduce(Text key, Iterable<Text> values, 
			...
			//遍历所有员工
			for(Text emp : values){
				empName = emp.toString().split(",")[0].trim();
				empEnterDate = emp.toString().split(",")[1].trim();
				try{
                   if(sdf.parse(empEnterDate).compareTo(earliestDate) < 0){
                	   earliestEmp = empName;
                	   earliestDate = sdf.parse(empEnterDate);
                   }					
				}catch(Exception e){
					e.printStackTrace();
				}
			}
			context.write(key, new Text("EmpName:" + earliestEmp + "  EnterDate:" + sdf.format(earliestDate)));
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), "OldestEmpNameOfDept");
		job.setJarByClass(OldestEmpNameOfDept.class);
	...
	public static void main(String[] args) throws Exception{
        ...
        "hdfs://node1:9000/inaction/home_task1/out/3"};
		Configuration config = new Configuration();
		config.addResource("hadoop-config.xml");
		int result = ToolRunner.run(config, new OldestEmpNameOfDept(), params);
		System.exit(result);
	}
