 ...
 *  10,ACCOUNTING,NEW YORK
	...
	在map输出中，利用不同的标记标记出 经理 和 员工，按部分划分，
	然后在reduce中遍历部门下的数据 对经理和员工数据分别处理
 * */
public class EmpOfMoreSalaryThanManager extends Configured implements Tool{
		...
		protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, Text>.Context context)
			...
			if(deptInfo.containsKey(empInfo[7])){
				if(null != empInfo[5] && !"".equals(empInfo[5].toString())){
					if("MANAGER".equals(empInfo[2])){
						context.write(new Text(deptInfo.get(empInfo[7])), new Text("M-" + empInfo[5]));
					}else{
						context.write(new Text(deptInfo.get(empInfo[7])), new Text("E-" + empInfo[1] + "-" + empInfo[5]));
					}
				}
			}
		}
	...
	private static class ReduceClass extends Reducer<Text, Text, Text, Text>{
		...
		protected void reduce(Text key, Iterable<Text> values, 
            ...
            Map<String, Long> empInfo = new HashMap<String, Long>();
            for(Text value : values){
            	String[] info = value.toString().split("-");
            	if("M".equals(info[0])){
            		managerSalary = Long.parseLong(info[1]);
            	}else{
            		empInfo.put(info[1], Long.parseLong(info[2]));
            	}
            }
            
            context.write(key, new Text("ManagerSalary:" + managerSalary));
            for(Map.Entry<String, Long> emp : empInfo.entrySet()){
            	if(emp.getValue() > managerSalary){
            		context.write(key, new Text("Name:" + emp.getKey() + ",Salary:" + emp.getValue()));
            	}
            }
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), "EmpOfMoreSalaryThanManager");
		job.setJarByClass(EmpOfMoreSalaryThanManager.class);
	...
	public static void main(String[] args) throws Exception{
        ...
        "hdfs://node1:9000/inaction/home_task1/out/5"};
		Configuration config = new Configuration();
		config.addResource("hadoop-config.xml");
		int result = ToolRunner.run(config, new EmpOfMoreSalaryThanManager(), params);
		System.exit(result);
	}
