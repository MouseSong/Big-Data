 ...
 * */
public class SumSalaryOfCity extends Configured implements Tool{
	...
	public static class MapClass extends Mapper<LongWritable, Text, Text, Text>{
		...
		protected void setup(Mapper<LongWritable, Text, Text, Text>.Context context)
			...
			try{
				...
				while((deptIdName = reader.readLine()) != null){
			    	deptInfo.put(deptIdName.split(",")[0], deptIdName.split(",")[2]);
			    }
			}catch(IOException e){
				e.printStackTrace();
			}finally{
	...
	private static class ReduceClass extends Reducer<Text, Text, Text, Text>{
		...
		protected void reduce(Text key, Iterable<Text> values, 
		    ...
		    context.write(key, new Text("SumSalary:" + String.valueOf(sumSalary)));
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(getConf(), "SumSalaryOfCity");
		job.setJarByClass(SumSalaryOfCity.class);
	...
	public static void main(String[] args) throws Exception{
        ...
        "hdfs://node1:9000/inaction/home_task1/out/4"};
		Configuration config = new Configuration();
		config.addResource("hadoop-config.xml");
		int result = ToolRunner.run(config, new SumSalaryOfCity(), params);
		System.exit(result);
	}
